...
const PropertiesAdd = () => {
    ...
    const fetchActiveCities = async () => {
      try {
        ...
        if (response.status === 200 && response.data && response.data.cities) {
          const formattedCities = response.data.cities.map((city) => ({
            value: city.id,
            label: `${city.title} ${city.TblCountry ? `(${city.TblCountry.countryName})` : ""
              }`.trim(),
          }));
          setCityOptions(formattedCities);
          console.log("Formatted Cities:", formattedCities);
  ...
  //   if (formData.city && cityOptions.length > 0 && typeof formData.city !== "object") {
  //     const selectedCity = cityOptions.find(
  //       (opt) => String(opt.value).toLowerCase() === String(formData.city).toLowerCase()
  //     );
  //     if (selectedCity) {
  ...
  const getProperty = async () => {
    try {
      ...
      const cityObj = Property.cities 
      ? { value: Property.cities.id, label: Property.cities.title } 
      : null;
  ...
  const validateForm = (formData) => {
    ...
    // }
    if (!formData.city || !formData.city.value) {
      errors.city = "City is required";
    }
                      ...
                      <div className=" ">
                        ...
                        <SelectComponent
                          ...
                          onChange={(selectedOption) =>
                            setFormData((prevData) => ({
                              ...prevData,
                              city: selectedOption, // Save full object { value, label }
                            }))
                          }
                          options={cityOptions || []}
                        />
